<?php session_start(); ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>TechZone | Chuột máy tính gần đây</title>
    <link rel="shortcut icon" href="picture/png-transparent-laptop-computer-icons-computer-desktop-pc-electronics-rectangle-computer.png" type="image/x-icon">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css">
    <link rel="stylesheet" href="styleforpr.css">

</head>
<body>
    <header>
        <a href="sau-khi-dang-nhap.php" class="logo">TechZone</a>

        <div class="search-bar" role="search" aria-label="Search site">
            <input type="checkbox" id="menu-toggle" hidden>
            <label for="menu-toggle" class="menu-btn" aria-hidden="true">
                <img src="picture/menu-burger.png" class="menu-icon" alt="menu">
            </label>

            <div class="dropdown-content" aria-hidden="true">
                <a href="sanpham-banphim.php">Bàn phím</a>
                <a href="sanpham-chuot.php">Chuột</a>
                <a href="sanpham-banphim.php">Tai nghe</a>
                <a href="sanpham-chuot.php">Màn hình</a>
            </div>

            <form action="timkiembanphim.php" method="GET" class="search-form">
                <label for="search-box" class="visually-hidden">Tìm kiếm</label>
                <input type="search" id="search-box" name="q" placeholder="Tìm kiếm sản phẩm, thương hiệu..." aria-label="Tìm kiếm" autocomplete="off">
                <button type="submit" class="search-submit" aria-label="Tìm kiếm">
                    <img src="picture/magnifying-glass.png" alt="">
                </button>
            </form>
        </div>
        <nav class="navbar">
        <a href="sau-khi-dang-nhap.php">Trang chủ</a> 
        <a href="#">Sản Phẩm</a> 
        <a href="#">Liên hệ</a>     
        </nav>

        <div class="icon">
            <div><a href="giohang.php" class="shopping-cart"><img src="picture/shopping.png" alt="Giỏ hàng"></a></div>

            <div class="user"> <a href="suathongtin.php" ><img class="avatar" src="picture/avatar-con-gian-cute.jpg" alt="Người dùng"><span>TÈO</span></a><a class="dangnhap"href="trangchu.php">Đăng xuất</a></div>
        </div>
    </header>
    <main class="container">
        <br>

        <div class="layout">
            <!-- Thanh bên trái -->
            <aside class="sidebar">
                <h3>Mức giá</h3>
                <label><input type="checkbox"> Dưới 500k</label><br>
                <label><input type="checkbox"> 500k - 1 triệu</label><br>
                <label><input type="checkbox"> 1 triệu - 2 triệu</label><br>
                <label><input type="checkbox"> Trên 2 triệu</label><br><br>
          
                <h3>Hãng</h3>
                <label><input type="checkbox"> Logitech</label><br>
                <label><input type="checkbox"> Razer</label><br>
                <label><input type="checkbox"> Corsair</label><br>
                <label><input type="checkbox"> AKKO</label><br>
                <label><input type="checkbox"> MSI</label><br>
              </aside>
    
        <section class="products">
            <h1>Mouse | Chuột máy tính</h1>
            <br>
            <div class="grid">
            <!-- Product 1 -->
            <article class="card">
            <div class="img"><a href="thongtinsanpham.php"><img src="picture/product2.1.png" alt="AULA SC580 Tri-Mode Wireless Gaming Mouse"></a></div>
            
            <h3>AULA SC580 Tri-Mode Wireless Gaming Mouse</h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>
            
            <!-- Product 2 -->
                <article class="card">
            <div class="img"><img src="picture/product2.2.png" alt="VXE R1 SE+ Tri-Mode Wireless Gaming Mouse"></div>
            <h3>VXE R1 SE+ Tri-Mode Wireless Gaming Mouse</h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>
        
    
            <!-- Product 3 -->
            <article class="card">
            <div class="img"><img src="picture/product2.3.png" alt="Ajazz AJ159 Dual Mode Gaming Mouse"></div>
            <h3>Ajazz AJ159 Dual Mode Gaming Mouse</h3>
            <div class="price">1,870,000 ₫ <span>1,999,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>

    
            <!-- Product 4 -->
            <article class="card">
            <div class="img"><img src="picture/product2.4.png" alt="YUNZII C2 Silent Silicone Wireless Gaming Mouse"></div>
            <h3>YUNZII C2 Silent Silicone Wireless Gaming Mouse</h3>
            <div class="price">1,389,000 ₫ <span>1,499,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>
            
        
            <!-- Product 5 -->
            <article class="card">
            <div class="img"><img src="picture/product2.5.png" alt="Razer Deathadder Essential White"></div>
            <h3>Razer Deathadder Essential White</h3>
            <div class="price">2,389,000 ₫ <span>2,499,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>
    
    
            <!-- Product 6 -->
            <article class="card">
            <div class="img"><img src="picture/product2.6.png" alt="Logitech G102 Gen II Lightsync | Đen, RGB, 8000 DPI"></div>
            <h3>Logitech G102 Gen II Lightsync | Đen, RGB, 8000 DPI </h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>

            <!-- Product 7 -->
            <article class="card">
            <div class="img"><img src="picture/product2.7.png" alt="MSI M99 Led RGB (Black)"></div>
            <h3>MSI M99 Led RGB (Black)</h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>

            <!-- Product 8 -->
            <article class="card">
            <div class="img"><img src="picture/product2.8.png" alt="Chuột không dây Corsair Darkstar RGB (CH-931A011-AP)"></div>
            <h3>Chuột không dây Corsair Darkstar RGB (CH-931A011-AP)</h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div> 
            </article>

            <!-- Product 9 -->
            <article class="card">
            <div class="img"><img src="picture/product2.9.png" alt="Steelseries Aerox 3 Wireless Snow Edition"></div>
            <h3>Steelseries Aerox 3 Wireless Snow Edition</h3>
            <div class="price">2,889,000 ₫ <span>3,534,000 ₫</span></div>
            <div class="actions">
            <a class="btn btn-primary" href="thongtinsanpham.php">Chi tiết</a>
            <a class="btn btn-outline" href="#">Thêm vào giỏ hàng</a>
            </div>
            </article>
            </div> 
            <div class="pagination">
                <a href="#" class="prev">« Trở về </a>
                <a href="#" class="page active">1</a>
                <a href="#" class="page">2</a>
                <a href="#" class="page">3</a>
                <a href="#" class="next">Tiếp tục »</a>
            </div>
        </section>
    </main>

</body>
</html>